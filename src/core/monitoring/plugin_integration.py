...
class MonitoredPlugin(ModifierPlugin):
    ...
    def check_prerequisites(self) -> bool:
        ...
        with self.monitor.trace_operation(f"{self.name}.check_prerequisites"):
            result = super().check_prerequisites()
            self.monitor.record_metric(
                f"plugin.{self.name}.prerequisites_passed", 1 if result else 0
            )
            return result
    ...
    def modify(self) -> bool:
        ...
        with self.monitor.trace_operation(
            f"plugin.{self.name}", plugin_name=self.name, plugin_version=self.version
        ) as op:
            try:
                # Update progress
                ...
                self.monitor.report.add_error(
                    self.name, e, {"plugin_name": self.name, "plugin_version": self.version}
                )
                raise
...
class MonitoredPluginManager(PluginManager):
    ...
    def execute(self, plugin_names=None) -> dict:
