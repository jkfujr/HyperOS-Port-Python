...
class Repacker:
    ...
    def _pack_erofs(self, part_name: str, src_dir: Path, img_output: Path, fs_config: Path, file_contexts: Path) -> None:
        """Pack EROFS image"""
        cmd: List[str] = [
            "mkfs.erofs",
            "-zlz4hc,9",
    ...
    def _pack_ext4(self, part_name: str, src_dir: Path, img_output: Path, fs_config: Path, file_contexts: Path, is_rw: bool) -> None:
    ...
    def _make_ext4_image(self, part_name: str, src_dir: Path, img_path: Path, size: int, inodes: int, fs_config: Path, file_contexts: Path, is_rw: bool) -> None:
    ...
    def pack_super_image(self) -> None:
        ...
        if not self.ctx.is_ab_device:
            ...
            base_args.extend(["--metadata-slots", "2", "--group", f"qti_dynamic_partitions:{super_size}", "-F"])
            partitions: List[str] = ["odm", "mi_ext", "system", "system_ext", "product", "vendor", "odm_dlkm", "vendor_dlkm", "system_dlkm", "product_dlkm"]
            for part in partitions:
                img_path: Path = self.ctx.target_dir / f"{part}.img"
                if img_path.exists():
                    size: int = img_path.stat().st_size
                    base_args.extend(["--partition", f"{part}:none:{size}:qti_dynamic_partitions", "--image", f"{part}={img_path}"])
        else:
            self.logger.info("Packing V-AB super.img")
            base_args.extend(["--virtual-ab", "--metadata-slots", "3", "--group", f"qti_dynamic_partitions_a:{super_size}", "--group", f"qti_dynamic_partitions_b:{super_size}", "-F"])
            ...
            for part in partitions:
                ...
                if img_path.exists():
                    ...
                    base_args.extend(["--partition", f"{part}_a:none:{size}:qti_dynamic_partitions_a", "--image", f"{part}_a={img_path}", "--partition", f"{part}_b:none:0:qti_dynamic_partitions_b"])
    ...
    def _generate_flash_script(self, super_image_path: Path) -> None:
        ...
        final_zip_name: str = f"{self.ctx.stock_rom_code}-hybrid-{self.ctx.target_rom_version}-{timestamp}.zip"
    ...
    def _patch_update_binary_for_a_only(self, script_path: Path) -> None:
        ...
        content: str = script_path.read_text(encoding='utf-8', errors='ignore')
        content = content.replace("boot_a", "boot").replace("boot_b", "boot").replace("dtbo_a", "dtbo").replace("dtbo_b", "dtbo")
        content = content.replace("bootctl set-active-boot-slot a", "")
        new_lines: List[str] = [line for line in content.splitlines() if "lptools unmap" not in line]
        script_path.write_text("\n".join(new_lines), encoding='utf-8')
    ...
    def _patch_update_binary_firmware(self, script_path: Path, firmware_dir: Path) -> None:
        ...
        for fw in fw_files:
            ...
            if "dtbo" in fw or "cust" in fw or fw == "boot.img": continue
    ...
    def pack_ota_payload(self) -> None:
        ...
        if self.product_out.exists(): shutil.rmtree(self.product_out)
        self.images_out.mkdir(parents=True, exist_ok=True)
        self.meta_out.mkdir(parents=True, exist_ok=True)
        for part in ["SYSTEM", "SYSTEM_EXT", "PRODUCT", "VENDOR", "ODM", "MI_EXT"]: (self.product_out / part).mkdir(exist_ok=True)

        for img in self.ctx.target_dir.glob("*.img"): shutil.copy2(img, self.images_out)
        if self.ctx.repack_images_dir.exists():
            for img in self.ctx.repack_images_dir.glob("*.img"): shutil.copy2(img, self.images_out)
        ...
        if device_custom_dir.exists():
            ...
            if dtbo: shutil.copy2(dtbo[0], self.images_out / "dtbo.img")
            rec: Path = device_custom_dir / "recovery.img"
            if rec.exists(): shutil.copy2(rec, self.images_out)
            init_boot: Path = device_custom_dir / "init_boot-kernelsu.img"
            if init_boot.exists(): shutil.copy2(init_boot, self.images_out / "init_boot.img")
    ...
    def _generate_meta_info(self) -> None:
        ...
        partition_list: List[str] = [img.stem for img in self.images_out.glob("*.img") if img.stem != "cust"]
        with open(self.meta_out / "ab_partitions.txt", "w") as f:
            for p in sorted(partition_list): f.write(f"{p}\n")

        super_size: int = self._get_super_size() 
        group_size: int = super_size - 1048576
        super_parts: List[str] = [p for p in partition_list if p in ["system", "vendor", "product", "system_ext", "odm", "mi_ext", "odm_dlkm", "vendor_dlkm", "system_dlkm", "product_dlkm"]]
        ...
        with open(self.meta_out / "misc_info.txt", "w") as f: f.write("recovery_api_version=3\nfstab_version=2\nab_update=true\n")
        with open(self.meta_out / "update_engine_config.txt", "w") as f: f.write("PAYLOAD_MAJOR_VERSION=2\nPAYLOAD_MINOR_VERSION=8\n")

    def _copy_build_props(self) -> None:
        """Copy build.prop of each partition to directories required by META structure"""
        mapping: Dict[str, str] = {"system": "SYSTEM", "product": "PRODUCT", "system_ext": "SYSTEM_EXT", "vendor": "VENDOR", "odm": "ODM"}
        for part_lower, part_upper in mapping.items():
            src_prop: Optional[Path] = self.ctx.get_target_prop_file(part_lower)
            if src_prop and src_prop.exists(): shutil.copy2(src_prop, self.product_out / part_upper / "build.prop")
            else: self.logger.warning(f"build.prop for {part_lower} not found.")
    ...
    def _run_ota_tool(self) -> None:
        ...
        if custom_tmp_dir.exists(): shutil.rmtree(custom_tmp_dir)
        ...
        try:
            self.shell.run([str(self.ota_tools_dir / "bin" / "ota_from_target_files"), "-v", "-k", str(key_path), str(self.product_out), str(output_zip)], env=env)
            md5: str = hashlib.md5(open(output_zip, 'rb').read()).hexdigest()[:10]
            final_path: Path = self.out_dir / f"{self.ctx.stock_rom_code}-ota_full-{self.ctx.target_rom_version}-{self.ctx.security_patch}-{timestamp}-{md5}-{self.ctx.port_android_version}.zip"
    ...
    def _get_super_size(self) -> int:
        ...
        for size, devices in size_map.items():
            if device_code in devices: return size
        return 9126805504
