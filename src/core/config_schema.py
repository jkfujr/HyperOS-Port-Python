...
class ConfigValidator:
    ...
    def validate(
        self, config_path: str, config_data: Optional[Dict] = None
    ) -> Tuple[bool, List[str]]:
        ...
        if errors and self.strict_mode:
            raise ConfigValidationError(f"Validation failed for {config_path}", errors)
    ...
    def _validate_schema(self, data: Any, schema: Dict, path: Path, prefix: str = "") -> List[str]:
        ...
        if schema_type == "object":
            ...
            if data not in schema.get("enum", []):
                errors.append(
                    f"{prefix or path}: Value '{data}' not in allowed values {schema.get('enum')}"
                )
