...
class LpMetadataExtent(LpMetadataBase):
    ...
    def __init__(self, buffer):
        self.num_sectors, self.target_type, self.target_data, self.target_source = struct.unpack(
...
class LpMetadataPartitionGroup(LpMetadataBase):
    ...
    def __init__(self, buffer):
        self.name, self.flags, self.maximum_size = struct.unpack(self._fmt, buffer[0 : self.size])
...
class Metadata:
    ...
    def get_offsets(self, slot_number: int = 0) -> List[int]:
        ...
        backup_offset = (
            base + self.geometry.metadata_max_size * self.geometry.metadata_slot_count + _tmp_offset
        )
...
class SparseImage:
    ...
    def unsparse(self):
        ...
        unsparse_file = Path(unsparse_file_dir / f"{Path(self._fd.name).stem}.unsparse.img")
...
class LpUnpack:
    def __init__(self, **kwargs):
        ...
        self._out_dir = kwargs.get("OUTPUT_DIR")
    ...
    def _read_metadata_header(self, metadata: Metadata):
        ...
        for index, offset in enumerate(offsets):
            ...
            header = LpMetadataHeader(self._fd.read(LpMetadataHeader.get_struct_size()))
            header.partitions = LpMetadataTableDescriptor(
                self._fd.read(LpMetadataTableDescriptor.get_struct_size())
            )
            header.extents = LpMetadataTableDescriptor(
                self._fd.read(LpMetadataTableDescriptor.get_struct_size())
            )
            header.groups = LpMetadataTableDescriptor(
                self._fd.read(LpMetadataTableDescriptor.get_struct_size())
            )
